...
# ── McpStatus Tool (removed from model surface) ──────────────────────────────────────
# NOTE: the mcp_status model tool was removed (diagnostic infrastructure, not a
# capability). MCP connection state remains via the mcp_status SSE event and
# load_all_tools() status data. The tests below were deleted with the tool.
