...
def test_save_load_oauth_state(tmp_db):
    ...
    save_oauth_state(tmp_db, "notion", "verifier123", "client456", "state789", "https://token.ep", "https://auth.ep")
...
def test_discover_notion_metadata_success():
    ...
    mock_pr = {"authorization_servers": ["https://auth.notion.com"]}
    mock_as = {
...
def test_start_notion_oauth_builds_auth_url(tmp_db):
    ...
    with (
        patch("nally.mcp.oauth.discover_notion_metadata", new_callable=AsyncMock, return_value=mock_discovery),
...
def test_exchange_notion_code_from_sqlite(tmp_db):
    ...
    save_oauth_state(
        tmp_db, "notion", "db_verifier", "db_client", "db_state", "https://db.token/ep", "https://db.auth/ep"
    )
...
def test_exchange_notion_code_uses_discovered_token_endpoint(tmp_db):
    ...
    _pending_pkce.pop("notion", None)
    save_oauth_state(tmp_db, "notion", "v", "c", "s", "https://discovered.token/ep", "https://auth/ep")
...
def test_start_google_oauth_builds_auth_url(tmp_db):
    ...
    from nally.mcp.oauth import GOOGLE_AUTH_ENDPOINT, start_google_oauth
...
def test_exchange_google_code_success(tmp_db):
    ...
    with patch.dict(os.environ, env, clear=False), patch("nally.mcp.oauth.httpx.AsyncClient") as MockClient:
        instance = AsyncMock()
        instance.post = AsyncMock(return_value=mock_resp)
        instance.__aenter__ = AsyncMock(return_value=instance)
        instance.__aexit__ = AsyncMock(return_value=False)
